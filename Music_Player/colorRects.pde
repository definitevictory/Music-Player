void Colouring () {
  ...
  //
  fill(green);
  ProgressBarTime=playlist[currentSong].position()/playlist[currentSong].length(); //
  //println(printinhour); test
  rect(tprogressX, tprogressY, tprogressWidth*ProgressBarTime, tprogressHeight);
  println(playlist[currentSong].position()/playlist[currentSong].length());
  println(playlist[currentSong].position());
  println (playlist[currentSong].length());
  // FIX THIS LATER FOR TIMER

...
void mousePressed() { //Listener
  ...
  if (mouseX>playX && mouseX<playX+playWidth && mouseY>playY && mouseY<playY+playHeight) {
    if (playlist.isPlaying()) {
      ...
      if (LoopTimes==1) {
        ...
        pause=false;
        looping =true;}
        else{playlist.play(); pause=false;}
      }
    }
  }
  ...
  if (mouseX>forwardX && mouseX<forwardX+forwardWidth && mouseY>forwardY && mouseY<forwardY+forwardHeight)  {// skipX, skipY, skipWidth, skipHeight
    ...
    //println(currentSong);
    if (playlist.position() >= 100 && playlist.position()<=playlist.length()*0.75*LoopTimes ) {
      playlist.skip(skip);
    }
    //
    if ((playlist.length()*0.75*LoopTimes)<playlist.position())
    {
  ...
  if (mouseX>LoopaddX && mouseX<LoopaddX+LoopaddWidth && mouseY>LoopaddY && mouseY<LoopaddY+LoopaddHeight &&  LoopMenu ==true) {
    LoopTimes+=1; pause=true; playlist.pause();
  }
